...
struct Monitor {
	...
	/* FIXME: This may cause problems in the future, any changes in the ammount
		 of tags may break the code... */
	//change 'LENGTH(tags)' to the actual number of tags you have (9 by def)
	//if you wish to move this below config.h
	// Pixmap tagmap[LENGTH(tags)]; // << this would be the right way of doing this
	Pixmap tagmap[9];
...
updatepreview(void)
{
	...
	for (m = mons; m; m = m->next) {
		/* When applying the patch 'dwm-tag-preview', I added the 'sidepad' var into the
		calculation to take into account the 'barpadding' patch. */
		m->tagwin = XCreateWindow(dpy, root,
				m->wx + sidepad, m->by + bh + 1.5 * sidepad, m->mw / scalepreview, m->mh / scalepreview, 0,
